<?php
require_once dirname(__DIR__, 2). '/Domain/Security/AccessRegistry.php';
...
final class Authorizer {
    ...
    public static function validateIAM(mixed $user): bool
    {
        ...
        if (isset(LocalCouncil::IDENTIFIERS[$orgID]) && (
                is_null($dept) ||
                is_null($position))
        ) return false;
        ...
        if (!isset(AccessRegistry::LEVELS[$user['PERMISSIONS']['access_level']])) return false;
        ...
        $expected = AccessRegistry::SCOPES[$user['PERMISSIONS']['access_level']];
    ...
    {
        if (!\src\__LEGACY\Application\IAM\Authorizer::validateIAM($user)) return false;
        $orgID = $user['IDENTITY']['org_id'];
        return
            $orgID === OSC::IDENTIFIER ||
            isset(LocalCouncil::IDENTIFIERS[$orgID]);
    }
    ...
    public static function isOSCExecutive(mixed $user): bool
    {
        ...
        $mapPosition = OSC::EXECUTIVE_MAP[$dept] ?? null;
    ...
    public static function isViewer(mixed $user): bool
    {
        ...
        $expected = array_flip(AccessRegistry::SCOPES['viewer']);
    ...
    private static function hasAdminClaims(mixed $user): bool
    {
        ...
        $expected = array_flip(AccessRegistry::SCOPES['admin']);
        $actual = array_flip($permissions['access_scope']);
        return
            empty(array_diff_key($expected, $actual)) &&
            empty(array_diff_key($actual, $expected));
    ...
    private static function hasEditorClaims(mixed $user): bool
    {
        ...
        $expected = array_flip(AccessRegistry::SCOPES['editor']);
        $actual = array_flip($permissions['access_scope']);
        return
            empty(array_diff_key($expected, $actual)) &&
            empty(array_diff_key($actual, $expected));
    ...
    public static function can(mixed $user, mixed $requirements): bool
    {
        ...
        foreach ($requiredScopes as $s) if (!in_array($s, AccessRegistry::SCOPES['common'], true)) return (
        ...
        $permissions = $user['PERMISSIONS'];
        $accessScope = $permissions['access_scope'] ?? AccessRegistry::SCOPES['viewer'];
    ...
    public static function canAuthor(mixed $docCategory, mixed $user): bool
    {
        ...
        $canAuthorCategory = match (strtoupper(trim($docCategory))) {
            default => false,
